  ...
  <div class="container">
    ...
    <table class="table table-condensed" v-if="community">
      ...
      <thead>
        ...
        <td style="width:35%" v-text="community.communityId"></td>
        <th style="width:10%">글쓴이</th>
        <td colspan="2" style="width:45%" v-text="community.name"></td>
      </thead>
	  <tbody>
      	<tr>
...
export default {
	...
	created(){
		axios
		.get(`http://localhost:8808/community/${this.$route.query.no}`)
		.then(({data}) =>{
			this.community = data;
		})
		.catch(() =>{
